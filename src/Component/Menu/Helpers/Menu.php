...
namespace Turbo\Speedwork\Component\Menu\Helpers;
...
class Menu extends Helper
{
    ...
    private function getMenuDetails($menu_type, $parents = false)
    {
        ...
        foreach ($result as $menuItem) {
            ...
            $menu['items'][$menuItem['id']] = $menuItem;
            // Creates entry into parents array. Parents array contains a list of all items with children
            $menu['parents'][$menuItem['parent_id']][] = $menuItem['id'];
        }
    ...
    public function buildMenuSelect($parent, $menu, $selected, $select_parents, $type, $level = 0)
    {
        ...
        if (isset($menu['parents'][$parent])) {
            ...
            foreach ($menu['parents'][$parent] as $itemId) {
                ...
                if ($input) {
                    $se       = (in_array($data['id'], $sel)) ? 'checked="checked"' : '';
                    $template = '<input type="'.$input_type.'" name="category[]" value="'.$data['id'].'" '.$se.'/>';
                } else {
    ...
    public function menuSelect($menu_type, $only_parents = false, $selected = '', $select_parents = true, $type = 0, $all = false)
    {
        ...
        if ($all === false) {
            $conditions[] = (!$this->is_user_logged_in) ? ['access <>' => 1] : ['access <>' => 2];
            $conditions[] = ($only_parents) ? ['parent_id' => 0] : '';
            $conditions[] = ['status' => 1];
        }
        ...
        return self::buildMenuSelect(0, $menu, $selected, $select_parents, $type);
    }

    public function menuIDToName($id, $implode = ',')
    {
        $res      = $this->database->find('#__core_menu', 'all', ['conditions' => ['id' => $id], 'fields' => ['name']]);
        $menuName = [];
        foreach ($res as $data) {
            $menuName[] = $data['name'];
        }
    ...
    public function menuId($cats = '')
    {
        ...
        if ($cats) {
            ...
            foreach ($menus as $menu) {
                $menu['items'][$menu['id']]            = $menu;
                $menu['parents'][$menu['parent_id']][] = $menu['id'];
            }
    ...
    public function menuIDToParents($id, $implode = ' &raquo; ')
    {
        ...
        $row = $this->database->find('#__core_menu', 'first', ['conditions' => ['id' => $id], 'fields' => ['name', 'parent_id']]);
        $m[] = $row['name'];
        $pid = $row['parent_id'];
    ...
    public function getParentLinks($id)
    {
        ...
        $res = $this->database->find('#__core_menu', 'all', [
            'conditions' => ['id' => $id],
            'fields'     => ['name', 'link', 'id', 'menu_type', 'parent_id'],
    ...
    public function hasPermission($link)
    {
        ...
        if ($parse['query']) {
            ...
            if ($parse['option'] && !$this->get('acl')->isGranted($rule)) {
                return false;
            }
        }
